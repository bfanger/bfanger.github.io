<script context="module">
  ...
  export async function preload({ params }) {
    const response = await this.fetch(`projects/${params.slug}.json`);
...
<Page>
  ...
  <div class="project-page__previous">
    {#if project.previous}
      <NavButton href="projects/{project.previous}" type="previous">
        Vorige
      </NavButton>
    {:else}
  ...
  {#if project.next}
    <div class="project-page__next">
      <NavButton href="projects/{project.next}" type="next">Volgende</NavButton>
    </div>
  {/if}
